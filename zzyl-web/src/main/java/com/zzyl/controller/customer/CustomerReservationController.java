...
public class CustomerReservationController extends BaseController {
    ...
    @GetMapping("/page")
    @ApiOperation("分页查询")
    public ResponseResult<PageResponse<ReservationVo>> findByPage(
            @ApiParam("页码") @RequestParam(defaultValue = "1", name = "pageNum") Integer pageNum,
            @ApiParam("每页数量") @RequestParam(defaultValue = "10", name = "pageSize") Integer pageSize,
            @ApiParam("每页数量") Integer status
    ) {
        PageResponse<ReservationVo> pageResponse = reservationService.page(pageNum,pageSize,status);
