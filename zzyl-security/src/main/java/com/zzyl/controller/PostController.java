...
public class PostController {
    ...
    @ApiOperation("岗位分页查询")
    public ResponseResult<PageResponse<PostVo>> findPostByPage(
            @RequestBody PostDto postDto,
            @RequestParam(defaultValue = "1") @PathVariable Integer pageNum,
            @RequestParam(defaultValue = "10") @PathVariable Integer pageSize
    ) {
        PageResponse<PostVo> pageResponse = postService.findPostByPage(postDto, pageNum, pageSize);
    ...
    public ResponseResult<Boolean> createPost(@RequestBody PostDto postDto) {
        boolean flag = postService.createPost(postDto);
    ...
    @ApiOperationSupport(includeParameters = {
            "postDto.id","postDto.deptNo", "postDto.dataState", "postDto.postName", "postDto.remark"
    })
    public ResponseResult<Boolean> updatePost(@RequestBody PostDto postDto) {
        boolean flag = postService.update(postDto);
        return ResponseResult.success(flag);
    }

    @DeleteMapping("/{postIds}")
    @ApiOperation("岗位删除")
    public ResponseResult<Boolean> deletePost(@PathVariable String postIds) {
        boolean flag = postService.remove(postIds);
        return ResponseResult.success(flag);
    }
